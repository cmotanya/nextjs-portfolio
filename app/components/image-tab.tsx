...
const Tabs = ({ children }: TabsProps) => {
    ...
    return children.map((child, index) => {
      if (index === isActiveTab) {
        return (
          <div
            key={index}
            className="mx-auto mt-4 flex min-h-[15rem] grid-rows-2 flex-row flex-wrap gap-3 justify-center"
          >
  ...
  return (
    <div className="mx-auto mt-14">
      <ul className="flex flex-row justify-center gap-8">{renderTabs()}</ul>
      {renderContents()}
    </div>
  );
};
